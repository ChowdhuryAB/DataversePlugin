...
namespace Apg.Shared.Core
{
    ...
    {
        public static string Name(this Entity entity, PluginPortfolio portfolio, bool includeEntityType)
        {
            var result = string.Empty;
            var priatt = portfolio.PrimaryAttribute(entity.LogicalName);
        ...
        public static void SetAttributeValue(this Entity entity, string fieldName, object value)
        {
            if (entity.Attributes.Contains(fieldName))
                entity.Attributes[fieldName] = value;
            else
                entity.Attributes.Add(new KeyValuePair<string, object>(fieldName, value));
        }
    }
}
